...
function allItemsInJob(job) { return job.bags.flatMap(b => b.items); }
...
// Tabbed emoji picker with wide selection
      ...
      <div style={{ padding: photo ? "0 20px 16px" : "16px 20px 16px", position: "relative" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
            <div style={{ fontSize: 26, width: 46, height: 46, background: "#222", borderRadius: 13, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "2px solid #2a2a2a" }}>{icon}</div>
...
function JobsView({ data, setData, navigate }) {
  ...
  const [form, setForm] = useState({ name: "", icon: "🎸", photo: null });

  const JOB_TABS = ["Music","Build","Art","Tech","Events","Sport","Health"];

  function openAdd() { setForm({ name: "", icon: "🎸", photo: null }); setEditJob(null); setShowAdd(true); }
  function openEdit(job, e) {
    e.stopPropagation();
    setForm({ name: job.name, icon: job.icon, photo: job.photo || null });
    setEditJob(job);
    setShowAdd(true);
  }
              ...
              <div style={{ padding: "14px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ fontSize: 24, width: 44, height: 44, background: "#222", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{job.icon}</div>
                    <div>
...
function BagsView({ data, setData, jobId, navigate }) {
  ...
  const [jobEditForm, setJobEditForm] = useState({ name: "", icon: "🎸", photo: null });
  ...
  function openAdd() { setForm({ name: "", icon: "🎒", photo: null }); setEditBag(null); setShowAdd(true); }
  function openEdit(bag, e) {
    e.stopPropagation();
    setForm({ name: bag.name, icon: bag.icon, photo: bag.photo || null });
    setEditBag(bag);
    setShowAdd(true);
  }
  ...
  function saveBag() {
    ...
    if (editBag) {
      ...
      setData(d => {
        let s = { ...d };
        let [id, ns] = generateId(s);
        ns.jobs = ns.jobs.map(j => j.id === jobId ? { ...j, bags: [...j.bags, { id, name: form.name.trim(), icon: form.icon, photo: form.photo, items: [] }] } : j);
        return ns;
      });
    }
    setShowAdd(false);
  ...
  function resetAll() {
    setData(d => ({ ...d, jobs: d.jobs.map(j => j.id !== jobId ? j : { ...j, bags: j.bags.map(b => ({ ...b, items: b.items.map(i => ({ ...i, checked: false })) })) }) }));
    setShowReset(false);
  }
      ...
      <PageHeader
        ...
        rightSlot={
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => { setJobEditForm({ name: job.name, icon: job.icon, photo: job.photo || null }); setShowEditJob(true); }} style={{ background: "#222", border: "1px solid #333", borderRadius: 10, padding: "8px 12px", color: "#aaa", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontSize: 12, flexShrink: 0 }}>
          ...
          return (
            <div key={bag.id} onClick={() => navigate("items", jobId, bag.id)} style={{ background: "#1a1a1a", borderRadius: 18, marginBottom: 12, border: "1px solid #252525", cursor: "pointer", overflow: "hidden" }}>
              ...
              <div style={{ padding: "12px 14px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ fontSize: 22, width: 40, height: 40, background: "#222", borderRadius: 11, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{bag.icon}</div>
                    <div>
                      <div style={{ fontSize: 16, fontWeight: 700 }}>{bag.name}</div>
                      <div style={{ fontSize: 12, color: "#555", marginTop: 2 }}>{bag.items.length} item{bag.items.length !== 1 ? "s" : ""}</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
...
function ItemsView({ data, setData, jobId, bagId, navigate }) {
  ...
  const [form, setForm] = useState({ name: "", emoji: "🔧", qty: 1, notes: "", photo: null });
  ...
  function openAdd() { setForm({ name: "", emoji: "🔧", qty: 1, notes: "", photo: null }); setEditItem(null); setShowAdd(true); }
  function openEdit(item, e) {
    e.stopPropagation();
    setForm({ name: item.name, emoji: item.emoji || "🔧", qty: item.qty, notes: item.notes || "", photo: item.photo || null });
    setEditItem(item);
    setShowAdd(true);
  }
    ...
    if (editItem) {
      ...
      setData(d => {
        ...
        ns.jobs = ns.jobs.map(j => j.id !== jobId ? j : {
          ...j, bags: j.bags.map(b => b.id !== bagId ? b : {
            ...b, items: [...b.items, { id, name: form.name.trim(), emoji: form.emoji, qty: form.qty, notes: form.notes, photo: form.photo, checked: false }]
          })
        });
        return ns;
      });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#111", color: "#f0f0f0", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        {bag.items.map(item => (
          <div key={item.id} style={{ background: item.checked ? "#181818" : "#1e1e1e", borderRadius: 14, marginBottom: 10, border: `1px solid ${item.checked ? "#222" : "#2c2c2c"}`, overflow: "hidden", transition: "background 0.2s, border-color 0.2s" }}>
            <div style={{ display: "flex", alignItems: "center", padding: "12px 12px 12px 0" }}>
              ...
              {/* Thumbnail: photo takes priority, else emoji */}
              {item.photo ? (
                ...
                <div style={{ fontSize: 26, width: 44, height: 44, background: "#222", borderRadius: 11, display: "flex", alignItems: "center", justifyContent: "center", marginRight: 10, flexShrink: 0, opacity: item.checked ? 0.35 : 1, transition: "opacity 0.2s" }}>{item.emoji || "🔧"}</div>
              )}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 16, fontWeight: 600, color: item.checked ? "#555" : "#f0f0f0", textDecoration: item.checked ? "line-through" : "none", transition: "color 0.2s", display: "flex", alignItems: "center", gap: 6 }}>
                  {/* Show emoji inline if there's also a photo */}
                  {item.photo && item.emoji && <span style={{ fontSize: 15, opacity: item.checked ? 0.5 : 1 }}>{item.emoji}</span>}
                  {item.name}
                </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4, marginLeft: 6 }}>
                <button onClick={e => openEdit(item, e)} style={{ background: "none", border: "none", color: "#555", cursor: "pointer", padding: 4 }}><Icon name="edit" size={15} /></button>
      ...
      {showAdd && (
        <Modal title={editItem ? "EDIT ITEM" : "ADD EQUIPMENT"} onClose={() => setShowAdd(false)}>
